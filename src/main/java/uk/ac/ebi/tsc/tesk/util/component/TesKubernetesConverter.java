...
public class TesKubernetesConverter {
    ...
    public V1Job fromTesExecutorToK8sJob(String generatedTaskId, String tesTaskName, TesExecutor executor, int executorIndex, TesResources resources, User user) {
        ...
        Optional.ofNullable(resources).map(TesResources::getRamGb).ifPresent(ramGb -> container.getResources().putRequestsItem(RESOURCE_MEM_KEY, (ramGb>0.007 ? String.format("%.3f", ramGb) : "0.007" ) + RESOURCE_MEM_UNIT));
        //
